  ...
  try {
    ...
    try {
      await prisma.webStoreOrder.create({
        data: {
          orderNumber: "", // Will be auto-generated by DB sequence/trigger (ZB-WEB-XXXXX)
          ...
          razorpayPaymentId: razorpay?.razorpay_payment_id || null,
          fulfillmentStatus: "unfulfilled",
          notes: paymentMethod === "COD" ? "COD Order from Web Store storefront" : "Paid via Razorpay from Web Store storefront",
          source: "web"
        }
      });
      console.log(`[Checkout] Successfully synced WebStoreOrder for localOrder: ${localOrder.id}`);
