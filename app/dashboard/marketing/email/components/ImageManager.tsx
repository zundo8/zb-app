...
export default function ImageManager({ htmlBody, onChange }: { htmlBody: string, onChange: (newHtml: string) => void }) {
  ...
  return (
    <div className="space-y-4 border-t border-white/10 pt-4 mt-4">
      <h3 className="text-sm font-medium text-white">Image Manager</h3>
      <div className="space-y-4">
        {slots.map((slot) => {
          const isUnsplash = slot.src.includes('unsplash.com');
          return (
            <div key={slot.id} className="bg-white/5 border border-white/10 rounded-xl p-4 flex gap-4 items-start">
              <div className="w-24 h-24 bg-black rounded-lg overflow-hidden shrink-0 border border-white/10">
                <img src={slot.src} alt={slot.alt} className="w-full h-full object-cover" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  <h4 className="text-sm font-medium text-white truncate">{slot.alt || `Image ${slot.index + 1}`}</h4>
                  {isUnsplash && (
                    <span className="text-[10px] bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded-full font-medium whitespace-nowrap">
                ...
                <div className="space-y-2 mt-3">
                  <input
                    ...
                    className="w-full bg-black/50 border border-white/10 rounded-md p-2 text-xs text-white placeholder-gray-500 focus:border-white/30 outline-none"
                  />
                  <button
                    onClick={() => setActiveSlot(slot.index)}
                    className="w-full bg-white/10 hover:bg-white/20 text-white text-xs py-2 rounded-md transition"
                  >
