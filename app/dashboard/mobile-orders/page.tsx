      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/20">Mobile Ecosystem</span>
          </div>
          <h1 className="text-4xl font-semibold tracking-tight text-white">App Orders</h1>
        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-white/5 p-1 rounded-xl border border-white/5">
            <button 
              ...
              className={`px-6 py-2 rounded-lg text-[11px] font-semibold transition-all ${tab === 'active' ? 'bg-white text-black shadow-sm' : 'text-white/40 hover:text-white'}`}
            >
              Active
            </button>
            <button 
              onClick={() => setTab('abandoned')}
              className={`px-6 py-2 rounded-lg text-[11px] font-semibold transition-all ${tab === 'abandoned' ? 'bg-white text-black shadow-sm' : 'text-white/40 hover:text-white'}`}
            >
              Abandoned
            </button>
          </div>
          <button onClick={fetchOrders} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl border border-white/5 transition-all">
      ...
      <div className="flex items-center gap-4 border-b border-white/5 pb-6">
        ...
        <div className="flex items-center gap-6">
           <div className="flex items-center gap-2">
             ...
             <span className="text-[11px] font-semibold text-white/40 uppercase tracking-widest">{orders.length} Records</span>
           </div>
        </div>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {loading ? (
            ...
            >
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/20">Syncing Node Data...</p>
            </motion.div>
          ) : orders.length === 0 ? (
            <motion.div 
              initial={{ opacity: 0 }} 
              animate={{ opacity: 1 }} 
              className="py-32 flex flex-col items-center justify-center space-y-4 border border-dashed border-white/5 rounded-3xl"
            >
              <Zap className="w-8 h-8 text-white/5" />
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/20">No active signals found</p>
            </motion.div>
          ) : (
            orders.map((order, i) => (
              <motion.div
              ...
              >
                <Link 
                  ...
                  className="grid grid-cols-12 gap-4 items-center px-6 py-4 rounded-2xl hover:bg-white/[0.03] border border-transparent hover:border-white/5 transition-all"
                >
                  <div className="col-span-3 flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[13px] font-semibold text-white tracking-tight">#{order.orderNumber || order.id.slice(-6).toUpperCase()}</p>
                      <p className="text-[10px] text-white/30 font-medium mt-0.5">{new Date(order.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</p>
                    </div>
                  </div>

                  <div className="col-span-3">
                    <p className="text-[12px] font-semibold text-white/80">{order.customer?.name || "Guest"}</p>
                  ...
                  <div className="col-span-2 text-right">
                    <div className="flex items-center justify-end gap-3 group/row">
                      <p className="text-[14px] font-semibold text-white tracking-tight">₹{order.totalPrice.toLocaleString()}</p>
