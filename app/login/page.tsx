                  ...
                  <div className="flex gap-2">
                    <div className="relative" ref={dropdownRef}>
                      <button type="button" onClick={() => setShowCountryDropdown(!showCountryDropdown)}
                        className="h-11 flex items-center gap-1.5 px-3.5 rounded-xl border border-white/10 bg-white/5 text-[10px] font-bold min-w-[70px] justify-between text-white/90 hover:bg-white/10 transition-all outline-none"
                      >
                      ...
                      <AnimatePresence>
                        {showCountryDropdown && (
                          <motion.div initial={{ opacity: 0, y: -5, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -5, scale: 0.98 }} transition={{ duration: 0.15 }}
                            className="absolute top-full left-0 mt-2 w-[180px] max-h-[200px] overflow-y-auto glass-liquid rounded-2xl shadow-2xl z-50 py-1.5 border border-white/5"
                          >
                    ...
                    <div className="relative flex-1">
                      <input type="tel" placeholder="10-digit number" value={phone} onChange={(e) => handlePhoneChange(e.target.value)}
                        className={`w-full h-11 px-4 rounded-xl border border-white/10 bg-white/5 text-[12px] font-bold tracking-widest text-white outline-none focus:border-white/20 transition-all ${phoneError ? '!border-red-500/30' : ''}`}
                ...
                <div className="flex justify-between gap-2 text-center px-1">
                  {otp.map((digit, idx) => (
                    <input key={idx} ref={(el) => { otpRefs.current[idx] = el; }} type="text" inputMode="numeric"
                      ...
                      className="w-11 h-11 text-center text-[13px] font-black rounded-xl border border-white/10 bg-white/5 text-white outline-none focus:border-white/20 transition-all placeholder:text-white/10" maxLength={1} required placeholder="•"
                    />
                  ))}
                </div>
          ...
          <div className="pt-2 space-y-4">
            ...
            <div className="flex gap-3">
              <button type="button" onClick={handleGoogleSignIn}
                className="flex-1 flex items-center justify-center h-11 rounded-xl border border-white/10 bg-white/5 text-white hover:bg-white/10 active:scale-95 transition-all"
