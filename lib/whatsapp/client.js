...
export async function sendTemplate({ to, templateName, languageCode = 'en_US', components = [] }) {
  ...
  try {
    ...
    // If Meta returns error 132000 (template language mismatch), try alternate 'en' / 'en_US' code
    if (error.code === 132000 || (error.message && error.message.includes('language'))) {
      const altLanguage = resolvedLanguage === 'en' ? 'en_US' : (resolvedLanguage === 'en_US' ? 'en' : 'en');
      console.warn(`[WhatsApp sendTemplate] Language "${resolvedLanguage}" failed with 132000 for "${templateName}". Retrying with "${altLanguage}"...`);
