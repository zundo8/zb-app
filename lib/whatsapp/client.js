 ...
 */
async function resolveTemplateLanguage(templateName, fallbackCode = 'en') {
  try {
...
export async function sendTemplate({ to, templateName, languageCode = 'en', components = [] }) {
  ...
  // Resolve the correct language code from DB cache to avoid Meta 132000 errors.
  // If the caller passed the default 'en', check if the template was actually
  // registered with a different code (e.g. 'en_US').
