...
async function sendAndLog({ phone, templateName, type, components, rawParams }) {
  ...
  const marketingTemplates = ['zica_cart_recovery_v1', 'zica_cart_recovery_v3', 'zb_abandoned_cart', 'zb_new_collection', 'zb_sale_alert', 'zb_restock_alert', 'zb_welcome'];
 ...
 * Template: zica_cart_recovery_v2 (MARKETING)
 * Enhanced with product image header and checkout button
 * Body: Hi {{1}},\nYou left {{2}} item(s) worth ₹{{3}} in your cart.\n\nComplete your purchase today.
 */
export async function sendAbandonedCart({ phone, customerName, checkoutUrl, productImageUrl, productName, cartTotal, itemCount }) {
  const components = [];

  // 1. Parse cartId from checkoutUrl
  let cartId = '';
  try {
  ...
  // 2. Fetch product handle from database to build dynamic storefront link
  let productUrl = 'https://app.zicabella.com/collections/all';
  if (cartId) {
    try {
      ...
      if (cartItem && cartItem.handle) {
        productUrl = `https://app.zicabella.com/products/${cartItem.handle}`;
      }
    } catch (e) {
      console.warn('[WhatsApp template] Failed to fetch cart item handle:', e.message);
    }
  }

  // IMAGE header with product image (if available)
  if (productImageUrl) {
    components.push({
      type: 'header',
      parameters: [
  ...
  // BODY variables: {{1}} customerName, {{2}} productName, {{3}} productUrl
  components.push({
    type: 'body',
    parameters: [
      { type: 'text', text: customerName || 'there' },
      { type: 'text', text: productName || 'items' },
      { type: 'text', text: productUrl }
    ]
  });

  // URL button with redirect pay link suffix: api/pay/[cartId]
  const suffix = `api/pay/${cartId}`;
  components.push({
    type: 'button',
    sub_type: 'url',
    index: 0,
  ...
  return sendAndLog({
    phone,
    templateName: 'zica_cart_recovery_v3',
    type: 'abandoned_cart',
    components,
 ...
 * Second reminder sent 4 hours after initial cart recovery — includes discount incentive
 */
 ...
 * Final reminder sent 24 hours after cart abandonment — urgency messaging
 */
 ...
 * Order tracking link with button to app.zicabella.com/orders/
 */
