...
export async function sendAbandonedCart({ phone, customerName, checkoutUrl, productImageUrl, productName, cartTotal, itemCount }) {
  ...
  if ((!resolvedImageUrl || resolvedImageUrl === '') && checkoutUrl) {
    try {
      let cartId = null;
      ...
      console.warn('[WhatsApp sendAbandonedCart] Failed to resolve product image from database:', err.message);
    }
  }
  ...
  return sendAndLog({
    ...
    type: 'abandoned_cart',
    components,
    rawParams: { customerName, checkoutUrl, productImageUrl: resolvedImageUrl, productName: resolvedName, cartTotal: resolvedTotal, itemCount: resolvedCount }
  });
}
