...
export async function sendAbandonedCart({ phone, customerName, checkoutUrl, productImageUrl, productName, cartTotal, itemCount }) {
  ...
  if (productImageUrl && (productImageUrl.startsWith('http://') || productImageUrl.startsWith('https://')) && !productImageUrl.includes('localhost') && !productImageUrl.includes('127.0.0.1')) {
    try {
      const mediaId = await uploadMediaFromUrl(productImageUrl);
      if (mediaId) {
        headerImageParam = {
          type: 'image',
          image: { id: mediaId }
