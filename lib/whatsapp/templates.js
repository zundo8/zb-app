...
const CART_RECOVERY_EVENT_TYPES = [
  'abandoned_cart',
  'cart_followup',
  'cart_final',
];

// Pure marketing templates use STRICT consent (require explicit opt-in)
...
// Strict-only marketing (NOT cart recovery) — requires explicit opt-in
...
const STRICT_MARKETING_EVENT_TYPES = [
  'new_collection',
  'sale_alert',
  'restock_alert',
  'welcome',
    ...
    if (bodyVarsCount > 0) {
      ...
      switch (eventType) {
        ...
        case 'exchange_pickup_scheduled':
          defaultSeq = [data.customerName, data.orderId, data.pickupDate || 'Scheduled', data.awbNumber || 'Assigned'];
          break;
        case 'exchange_confirmed':
          defaultSeq = [data.customerName, data.orderId, data.newItemName || 'Replacement Item'];
          break;
        case 'exchange_shipped':
          defaultSeq = [data.customerName, data.orderId, data.trackingNumber || 'Pending'];
          break;
        case 'abandoned_cart':
          defaultSeq = [data.customerName, data.productName || data.checkoutUrl, data.checkoutUrl];
          break;
        case 'cart_followup':
        case 'cart_final':
          defaultSeq = [data.customerName, data.discountCode || 'ZICA10', data.checkoutUrl];
          break;
        case 'new_collection':
          defaultSeq = [data.customerName, data.collectionName || 'New Collection', data.tagline || 'Shop Now'];
          break;
        case 'sale_alert':
          defaultSeq = [data.customerName, String(data.discountPercent || '10'), data.saleEndDate || 'Limited Time'];
          break;
        case 'restock_alert':
          defaultSeq = [data.customerName, data.productName || 'Favorite Item', data.size || 'All Sizes'];
          break;
        case 'welcome':
          defaultSeq = [data.customerName || 'there'];
          break;
        case 'account_created':
          defaultSeq = [data.customerName || 'there'];
          break;
        default:
          defaultSeq = [data.customerName, data.orderId || data.checkoutUrl];
      }

      for (let i = 0; i < bodyVarsCount; i++) {
        let val = defaultSeq[i];
        if (val === undefined || val === null || String(val).trim() === '') {
          if (i === 0) val = data.customerName || 'Customer';
          else if (i === 1) val = data.productName || data.orderId || 'Zica Bella';
          else val = 'Shop Now';
        }
        params.push({
          type: 'text',
 ...
 */
export async function sendOrderConfirmation({ phone, customerName, orderId, productImageUrl, orderStatusUrl }) {
  ...
  if (dynamicComponents !== null) {
    return sendAndLog({
      ...
      rawParams: { customerName, orderId, productImageUrl, orderStatusUrl }
    });
  }
